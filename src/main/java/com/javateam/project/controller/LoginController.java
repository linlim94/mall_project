...
public class LoginController {
	...
	public String loginProc(@RequestParam("id") String id, 
		...
		String page = "";
		// 로그인 창에서 아이디나 패스워드를 입력하지 않았을 경우
		if(id.trim() == "" || pwd.trim() == "") {
			
			// 다시 로그인 페이지로 이동
			model.addAttribute("err_msg", "아이디와 패스워드를 모두 입력해주세요.");
			model.addAttribute("move_path", "/login/login_form");
			page = "/error/error";
			
		
			// 탈퇴한 회원 여부 점검(useyn='n')
			
			// 로그인에 실패한 회원이 이전에 탈퇴한 회원인 경우
		} else if (memberService.isActiveMember(id) == false) {
			// 가입 페이지로 이동
			if(memberService.isMember(id) == true) {
				...
				model.addAttribute("err_msg", memberService.checkLogin(id, pwd));
				model.addAttribute("move_path", "/member/join_form");
				page = "/error/error";
			}
